...
public class LogServiceAspect {
	...
	public void beforeMethod(JoinPoint joinpoint) {
		 ...
		 * joinpoint.getSignature().getName();// 获取方法名 Class<?> targetClass =
		 * joinpoint.getTarget().getClass();// 获取目标对象的类名 Method method = null;
		 * for (Method mt : targetClass.getMethods()) { if
		 * (methodName.equals(mt.getName())) { method = mt; break; } } //level
		 * String level = ""; if(method.getName().contains("Controller")){
		 * level="Controller"; }else if(method.getName().contains("Service")){
		 * level="Service"; }else if(method.getName().contains("Dao")){
		 * level="Dao"; } //para String[] names = ((CodeSignature)
		 * joinpoint.getSignature()).getParameterNames(); Object[] args =
		 * joinpoint.getArgs(); StringBuilder sb = new StringBuilder("para: ");
		 * for (int i = 0; i < names.length; i++) { sb.append(names[i] + " = " +
		 * args[i] + ","); } //result
		 * 
		 * Log log = new Log(); log.setId(Math.random()+"");
		 * log.setLevel(level); log.setMethodName(method.getName());
		 * log.setPara(sb.toString()); try { logAspectMapper.insertLog(log); }
